let DEFAULT_SECTION_ORDER=["objectives","summary","keyPoints","images","questions"];function _getSectionOrder(s){if(s.sectionOrder&&0<s.sectionOrder.length){let e=[...s.sectionOrder];return(s.customSections||[]).forEach(s=>{e.includes(s.id)||e.splice(0<=e.indexOf("questions")?e.indexOf("questions"):e.length,0,s.id)}),e}let i=[...DEFAULT_SECTION_ORDER];return(s.customSections||[]).forEach(s=>{var e=i.indexOf("questions");i.splice(0<=e?e:i.length,0,s.id)}),i}function _renderSection(e,s,i,t,n,c){switch(e){case"objectives":return s.objectives&&s.objectives.length?`<div class="lp-section">
    ...
    ${s.map((s,e)=>`<div class="lp-kp-card" style="animation-delay:${.06*e}s;--kp-color:${t}">
      <div class="lp-kp-dot" style="background:${t}"></div><span>${s}</span></div>`).join("")}
    ...
    ${s.map((s,e)=>`<div class="lp-kp-card${90<s.replace(/<[^>]*>/g,"").trim().length?" kp-full":""}" style="animation-delay:${.06*e}s;--kp-color:${t}">
        <div class="lp-kp-num" style="background:${t};color:#fff">${e+1}</div><span>${s}</span></div>`).join("")}</div>`;if("cards"!==e)return"";{let i=[t,"#2196F3","#10B981","#F59E0B","#EC4899","#7C3AED"];return`<div class="lp-keypoints lp-kp-cards">
    ${s.map((s,e)=>`<div class="lp-kp-ccard" style="--card-color:${i[e%i.length]};animation-delay:${.07*e}s">
        <div class="lp-kp-ccard-num">${e+1}</div>
        <div class="lp-kp-ccard-body">${s}</div></div>`).join("")}</div>`}}function renderLessonImages(s,e){s=(s.images||[]).filter(s=>s.src);return s.length?`<div class="lp-section">
    <div class="lp-section-header" style="--sec-color:${e}">
      <div class="lp-section-icon">🖼️</div><h2>صور الدرس</h2>
    </div>
    <div class="lp-img-gallery">
      ${s.map(s=>`<div class="lp-img-item">
        <img src="${s.src}" alt="${s.caption||""}" loading="lazy">
        ${s.caption?`<div class="lp-img-caption">${s.caption}</div>`:""}
      </div>`).join("")}
    </div>
  </div>`:""}function _initLessonFocus(n){let c=[...document.querySelectorAll(".lp-focus-card")];if(c.length){var s=document.getElementById("lp-prog-dots");let i=document.getElementById("lp-prog-label"),t=document.getElementById("lp-progress-bar");if(s&&(s.innerHTML=c.map((s,e)=>`<span class="lp-dot" id="lp-dot-${e}"></span>`).join("")),c.forEach(s=>s.classList.add("lp-sec-dimmed")),c[0]&&(c[0].classList.remove("lp-sec-dimmed"),c[0].classList.add("lp-sec-active"),_updateProgBar(c[0],0,i,t,n)),"IntersectionObserver"in window){window._lessonSecObs&&window._lessonSecObs.disconnect();let e=new IntersectionObserver(s=>{s.forEach(s=>{var e;s.isIntersecting&&.2<=s.intersectionRatio&&(c.forEach(s=>{s.classList.add("lp-sec-dimmed"),s.classList.remove("lp-sec-active")}),s.target.classList.remove("lp-sec-dimmed"),s.target.classList.add("lp-sec-active"),e=+s.target.dataset.secIndex,_updateProgBar(s.target,e,i,t,n))})},{threshold:.2,rootMargin:"-8% 0px -42% 0px"});c.forEach(s=>e.observe(s)),window._lessonSecObs=e}}}function _updateProgBar(s,i,e,t,n){e&&(e.textContent=s.dataset.secLabel||""),document.querySelectorAll(".lp-dot").forEach((s,e)=>{s.classList.toggle("lp-dot-active",e===i)})}document.addEventListener("DOMContentLoaded",()=>{var s=new URLSearchParams(location.search);let e=s.get("g"),i=s.get("s"),t=s.get("u"),n=s.get("l");if(window.FirebaseDB&&window.FirebaseDB.dbLoadCurriculum){let e=window.FirebaseDB.dbLoadCurriculum();e&&Object.keys(e).length&&Object.keys(e).forEach(s=>CURRICULUM[s]=e[s])}s=localStorage.getItem("ibn_moshrf_curriculum");if(s)try{let e=JSON.parse(s);Object.keys(e).forEach(s=>CURRICULUM[s]=e[s])}catch(s){}let c=CURRICULUM[e];s=c.semesters.find(s=>s.id===i);let l=s.units.find(s=>s.id===t),a=l.lessons.find(s=>s.id===n),o=(document.title=a.name+" - المهارات الرقمية",window._currentLessonId=n,window._currentLessonGrade=e,document.getElementById("breadcrumb").innerHTML=`
    ...
    <span class="bc-sep">›</span>
    <span class="bc-item bc-current">${a.name}</span>`,l.color),d=JSON.stringify(a.questions).replace(/"/g,"&quot;");var r=_getSectionOrder(a);let p={objectives:"🎯 أهداف الدرس",summary:"📋 ملخص الدرس",keyPoints:"⚡ النقاط الرئيسية",images:"🖼️ صور الدرس",questions:"🎮 اختبر نفسك"};r=r.map((e,s)=>{var i,t=_renderSection(e,a,l,c,o,d);return t?(i=(a.customSections||[]).find(s=>s.id===e),i=p[e]||(i?`${i.icon||"📌"} `+(i.title||"قسم"):""),t.replace(/^<div class="lp-section/,`<div class="lp-section lp-focus-card" data-sec-label="${i}" data-sec-index="${s}"`)):""}).join("");document.getElementById("main-content").innerHTML=`
