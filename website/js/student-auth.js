...
_clearLockData(username);_setCurrent(student);return{ok:true,student};}
...
function awardPoints(correct,total,lessonId){const cur=getCurrent();if(!cur)return null;const db=_load();const student=db[cur.id];if(!student)return null;let earned=correct*PTS_PER_CORRECT;if(correct===total&&total>0)earned+=COMPLETION_BONUS;const isRetry=!!(lessonId&&student.lessons&&student.lessons[lessonId]);if(isRetry)earned=Math.floor(earned*0.5);if(earned<0)earned=0;student.points=(student.points||0)+earned;if(lessonId){student.lessons=student.lessons||{};const prev=student.lessons[lessonId];if(!prev||correct>=(prev.correct||0)){student.lessons[lessonId]={correct,total,date:new Date().toISOString().slice(0,10)};}}
db[cur.id]=student;_save(db);updateHeaderBtn();updateStudentBar();if(window._refreshLeaderboard)window._refreshLeaderboard();return{earned,totalPoints:student.points,isRetry};}
...
function updateStudentBar(){const bar=document.getElementById('student-bar');if(!bar)return;const cur=getCurrent();const full=cur?getCurrentFull():null;bar.classList.add('sb-visible');if(cur&&full){const gc=GRADE_COLORS[full.grade]||'#3B82F6';const gn=GRADE_NAMES[full.grade]||full.grade;const gi=GRADE_ICONS[full.grade]||'📚';const lb=getLeaderboard(100);const rank=(lb.find(r=>r.id===full.id)||{}).rank;const rankVal=rank===1?'🥇 الأول':rank===2?'🥈 الثاني':rank===3?'🥉 الثالث':rank?`#${rank}`:'—';bar.innerHTML=`
        <div class="sb-card">
...
init();return{getCurrent,getCurrentFull,login,logout,addStudent,deleteStudent,updateStudent,getAllStudents,resetPoints,awardPoints,getLeaderboard,updateHeaderBtn,updateStudentBar,showPointsToast,_doBarLogin};})();