...
function _load(){if(window.FirebaseDB){const cache=window.FirebaseDB.dbLoad();if(Object.keys(cache).length>0)return cache;}
try{return JSON.parse(localStorage.getItem(STUDENTS_KEY))||{};}catch{return{};}}
function _save(d){try{localStorage.setItem(STUDENTS_KEY,JSON.stringify(d));}catch(e){}
...
_clearLockData(username);_setCurrent(student);const visitData={visitCount:(student.visitCount||0)+1,lastVisit:new Date().toISOString().slice(0,10)};if(window.FirebaseDB&&window.FirebaseDB.isFirebaseActive&&window.FirebaseDB.isFirebaseActive()){window.FirebaseDB.dbUpdateStudent(student.id,visitData);try{const ls=JSON.parse(localStorage.getItem(STUDENTS_KEY))||{};if(ls[student.id])Object.assign(ls[student.id],visitData);localStorage.setItem(STUDENTS_KEY,JSON.stringify(ls));}catch(e){}}else{const db2=_load();if(db2[student.id]){Object.assign(db2[student.id],visitData);_save(db2);}}
...
function awardPoints(correct,total,lessonId,lessonGradeId){const cur=getCurrent();if(!cur)return null;const db=_load();const student=db[cur.id];if(!student)return null;const gradeMatch=!lessonGradeId||(student.grade===lessonGradeId);if(!gradeMatch){student.attemptCount=(student.attemptCount||0)+1;db[cur.id]=student;if(window.FirebaseDB&&window.FirebaseDB.isFirebaseActive&&window.FirebaseDB.isFirebaseActive()){window.FirebaseDB.dbUpdateStudent(cur.id,{attemptCount:student.attemptCount});try{localStorage.setItem(STUDENTS_KEY,JSON.stringify(db));}catch(e){}}else{_save(db);}
return{earned:0,totalPoints:student.points||0,isRetry:false,wrongGrade:true};}
...
db[cur.id]=student;if(window.FirebaseDB&&window.FirebaseDB.isFirebaseActive&&window.FirebaseDB.isFirebaseActive()){window.FirebaseDB.dbUpdateStudent(cur.id,{points:student.points,lessons:student.lessons||{},attemptCount:student.attemptCount});try{localStorage.setItem(STUDENTS_KEY,JSON.stringify(db));}catch(e){}}else{_save(db);}
updateHeaderBtn();updateStudentBar();if(window._refreshLeaderboard)window._refreshLeaderboard();return{earned,totalPoints:student.points,isRetry,wrongGrade:false};}
...
async function _doBarLogin(){const nameEl=document.getElementById('sb-username');const passEl=document.getElementById('sb-password');const errEl=document.getElementById('sb-err-msg');const btn=document.querySelector('.sb-login-btn');if(!nameEl||!passEl)return;if(btn)btn.disabled=true;const res=await StudentAuth.login(nameEl.value,passEl.value);if(btn)btn.disabled=false;if(!res.ok){if(errEl){errEl.textContent='⚠ '+res.msg;errEl.classList.remove('hidden');}
