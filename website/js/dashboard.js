...
objectives:{icon:'🎯',title:'الأهداف التعليمية'},
summary:{icon:'📝',title:'ملخص الدرس'},
keyPoints:{icon:'⚡',title:'النقاط الرئيسية'},
images:{icon:'🖼️',title:'صور الدرس'},
questions:{icon:'🎮',title:'الأسئلة التفاعلية'},
};
function _getLessonSectionOrder(lesson){
if(lesson.sectionOrder&&lesson.sectionOrder.length>0){
const order=[...lesson.sectionOrder];

(lesson.customSections||[]).forEach(cs=>{
if(!order.includes(cs.id)){
const qi=order.indexOf('questions');
order.splice(qi>=0?qi:order.length,0,cs.id);
}
});
return order;
}
...
function renderSectionsOrderPanel(lesson){
const order=_getLessonSectionOrder(lesson);
return order.map((key,i)=>{
const builtin=BUILTIN_SECTIONS[key];
const custom=(lesson.customSections||[]).find(cs=>cs.id===key);
const icon=builtin?builtin.icon:(custom?custom.icon:'📌');
const title=builtin?builtin.title:(custom?custom.title||'قسم بدون عنوان':key);
return `
...
lesson.sectionOrder=order;
_reRenderSectionsOrder();
}
function removeSectionFromOrder(key){
const{lesson}=getLesson(activeLesson);
const order=_getLessonSectionOrder(lesson);
lesson.sectionOrder=order.filter(k=>k!==key);
if(lesson.customSections){
lesson.customSections=lesson.customSections.filter(cs=>cs.id!==key);
}
renderLessonEditor();
}
function addSectionFromOrder(){
const{lesson}=getLesson(activeLesson);
const id='cs_'+Date.now();

const order=_getLessonSectionOrder(lesson);
if(!lesson.customSections)lesson.customSections=[];
lesson.customSections.push({id,title:'',icon:'📌',content:''});
const qi=order.indexOf('questions');
order.splice(qi>=0?qi:order.length,0,id);
lesson.sectionOrder=order;
renderLessonEditor();
...
function _reRenderSectionsOrder(){
const panel=document.getElementById('sections-order-list');
if(!panel)return;
const{lesson}=getLesson(activeLesson);
panel.innerHTML=renderSectionsOrderPanel(lesson);
}
let qbState={};
...
const PIN_LOCK_KEY='ibn_pin_lockout';
const PIN_MAX_ATTEMPTS=5;
const PIN_LOCKOUT_MS=10*60*1000;
function _getPinLock(){
try{return JSON.parse(sessionStorage.getItem(PIN_LOCK_KEY))||{attempts:0};}
catch{return{attempts:0};}
}
function _savePinLock(d){sessionStorage.setItem(PIN_LOCK_KEY,JSON.stringify(d));}
function _checkPinLock(){
const d=_getPinLock();
if(d.until&&Date.now()<d.until){
const mins=Math.ceil((d.until-Date.now())/60000);
return{locked:true,msg:`تم تجميد الوصول لمدة ${mins}دقيقة بعد محاولات خاطئة متكررة`};
}
return{locked:false,remaining:PIN_MAX_ATTEMPTS-(d.attempts||0)};
}
function _recordPinFail(){
const d=_getPinLock();
if(d.until&&Date.now()>=d.until){d.attempts=0;d.until=null;}
d.attempts=(d.attempts||0)+1;
if(d.attempts>=PIN_MAX_ATTEMPTS){d.until=Date.now()+PIN_LOCKOUT_MS;d.attempts=0;}
_savePinLock(d);
}
...
const lock=_checkPinLock();
if(lock.locked){
if(errEl){errEl.textContent=lock.msg;errEl.classList.remove('hidden');}
...
if(hashed!==stored){
_recordPinFail();
const lockNow=_checkPinLock();
const msg=lockNow.locked
?lockNow.msg
:`رمز الدخول غير صحيح — ${lockNow.remaining}محاولة متبقية`;
...
try{
const quick=JSON.parse(localStorage.getItem('ibn_moshrf_teacher_quick')||'null');
if(quick&&quick.ts&&(Date.now()-quick.ts)<60000){
localStorage.removeItem('ibn_moshrf_teacher_quick');
sessionStorage.setItem(TEACHER_SESSION,'1');
_initDashboard();
return;
}
...
document.getElementById('teacher-account-panel').classList.add('hidden');
}
...
function showTeacherAccountPanel(){
document.getElementById('overview-panel').classList.add('hidden');
document.getElementById('lesson-editor').classList.add('hidden');
document.getElementById('student-panel').classList.add('hidden');
document.getElementById('teacher-account-panel').classList.remove('hidden');
renderTeacherAccountPanel();
}
function renderTeacherAccountPanel(){
const panel=document.getElementById('teacher-account-panel');
if(!panel)return;
const existing=window.StudentAuth&&window.StudentAuth.getTeacherAccount
?window.StudentAuth.getTeacherAccount()
:null;
panel.innerHTML=`
<div class="sp-header">
...
${existing?`<div style="background:#f0fdf4;border:1px solid #86efac;border-radius:10px;padding:14px 16px;margin-bottom:18px;font-size:14px;color:#166534">
✅ يوجد حساب معلم حالياً — الاسم:<strong>${existing.username}</strong><br>
...
const un=(document.getElementById('ta-username').value||'').trim();
const pw=(document.getElementById('ta-password').value||'').trim();
const pw2=(document.getElementById('ta-password2').value||'').trim();
const msgEl=document.getElementById('ta-msg');
...
if(!un)return showMsg('⚠️ يرجى إدخال اسم المستخدم',false);
if(!pw)return showMsg('⚠️ يرجى إدخال كلمة المرور',false);
if(pw!==pw2)return showMsg('⚠️ كلمتا المرور غير متطابقتين',false);
if(pw.length<4)return showMsg('⚠️ كلمة المرور يجب أن تكون 4 أحرف على الأقل',false);
if(window.StudentAuth&&window.StudentAuth.setTeacherAccount){
await window.StudentAuth.setTeacherAccount(un,pw);
showMsg('✅ تم حفظ حساب المعلم بنجاح!يمكنك الآن تسجيل الدخول من الموقع الرئيسي',true);
document.getElementById('ta-password').value='';
document.getElementById('ta-password2').value='';
setTimeout(renderTeacherAccountPanel,1500);
}
}
let _spFilter={grade:'all',category:'all',sort:'points'};
...
const catLabels={excellent:'🌟 متفوق',good:'👍 جيد',normal:'📘 عادي',needs_help:'⚠️ يحتاج دعم'};
const catColors={excellent:'#16a34a',good:'#2196F3',normal:'#64748b',needs_help:'#ef4444'};

let all=StudentAuth.getAllStudents();
all.forEach(st=>{
const pts=st.points||0;
const lc=Object.keys(st.lessons||{}).length;
if(!st.category||st.category==='normal'){
if(pts>=200||lc>=10)st.category='excellent';
else if(pts>=100||lc>=5)st.category='good';
else if(pts===0&&lc===0)st.category='needs_help';
else st.category='normal';
}
});

let students=all.filter(st=>{
if(_spFilter.grade!=='all'&&st.grade!==_spFilter.grade)return false;
if(_spFilter.category!=='all'&&(st.category||'normal')!==_spFilter.category)return false;
return true;
});

if(_spFilter.sort==='points')students.sort((a,b)=>(b.points||0)-(a.points||0));
if(_spFilter.sort==='visits')students.sort((a,b)=>(b.visitCount||0)-(a.visitCount||0));
if(_spFilter.sort==='attempts')students.sort((a,b)=>(b.attemptCount||0)-(a.attemptCount||0));
if(_spFilter.sort==='lessons')students.sort((a,b)=>Object.keys(b.lessons||{}).length-Object.keys(a.lessons||{}).length);
if(_spFilter.sort==='name')students.sort((a,b)=>a.username.localeCompare(b.username,'ar'));
const totalPts=all.reduce((s,st)=>s+(st.points||0),0);
const countExcell=all.filter(s=>s.category==='excellent').length;
const countNeed=all.filter(s=>s.category==='needs_help').length;
const countByGrade={grade4:0,grade5:0,grade6:0};
all.forEach(st=>{if(countByGrade[st.grade]!==undefined)countByGrade[st.grade]++;});
panel.innerHTML=`
<div class="sp-dash-header">
<div>
<h2 class="sp-dash-title">👥 إدارة الطلاب</h2>
<p class="sp-dash-sub">${all.length}طالب مسجّل · إجمالي النقاط:${totalPts}</p>
</div>
...
<option value="grade4" ${_spFilter.grade==='grade4'?'selected':''}>🟠 الصف الرابع</option>
<option value="grade5" ${_spFilter.grade==='grade5'?'selected':''}>🔵 الصف الخامس</option>
<option value="grade6" ${_spFilter.grade==='grade6'?'selected':''}>🟣 الصف السادس</option>
</select>
</div>
...
<option value="name" ${_spFilter.sort==='name'?'selected':''}>🔤 الاسم</option>
</select>
</div>
<div class="field-group" style="flex:0;padding-top:24px">
...
${students.length===0
?`<div class="sp-empty">لا يوجد طلاب بهذه الفلترة</div>`
...
onchange="setStudentCategory('${escHtml(st.id)}',this.value)">
...
<span class="sp-pass-dots" id="pd-${escHtml(st.id)}">••••</span>
...
<td class="sp-lc-cell">${st.visitCount||0}</td>
<td class="sp-lc-cell">${st.attemptCount||0}</td>
<td class="sp-actions-cell">
<button class="sp-reset-btn" onclick="resetStudentFromDash('${escHtml(st.id)}')" title="صفر النقاط">🔄 صفر</button>
<button class="sp-del-btn" onclick="deleteStudentFromDash('${escHtml(st.id)}')" title="حذف الطالب">🗑</button>
</td>
</tr>`;
}).join('')}
</tbody>
...
function setStudentCategory(id,cat){
if(!window.StudentAuth)return;
StudentAuth.updateStudent(id,{category:cat});
showToast('✅ تم تحديث تصنيف الطالب');
}
...
const existingImg=existing?.image||null;
qbState.matchImage=existingImg;
content.innerHTML=`
<div class="qb-field">
...
<img src="${existingImg}" alt="صورة السؤال">
...
🖼️ رفع صورة
<input type="file" accept="image
function handleMatchImageUpload(input){
const file=input.files[0];
if(!file)return;
const reader=new FileReader();
reader.onload=function(e){
qbState.matchImage=e.target.result;
_renderMatchImagePreview(e.target.result);
};
reader.readAsDataURL(file);
}
function setMatchImageFromURL(){
const url=(document.getElementById('qb-img-url')?.value||'').trim();
if(!url){showToast('⚠️ أدخل رابط الصورة');return;}
qbState.matchImage=url;
_renderMatchImagePreview(url);
}
function removeMatchImage(){
qbState.matchImage=null;
const area=document.getElementById('qb-img-area');
if(!area)return;
area.innerHTML=`
<div class="qb-img-placeholder" id="qb-img-placeholder">
<label class="qb-img-upload-btn">
...
function _renderMatchImagePreview(src){
const area=document.getElementById('qb-img-area');
if(!area)return;
area.innerHTML=`
<div class="qb-img-preview" id="qb-img-preview">
...
q={...q,pairs};
if(qbState.matchImage)q.image=qbState.matchImage;
}else if(type==='fill'){
const sentences=[];
const count=qbState.fillCount||0;
for(let i=0;i<count;i++){
